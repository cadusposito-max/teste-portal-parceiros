...
function userCanAccessAdminPanel() {
  // Gestores não precisam de MFA — acesso permanece como antes.
  if (state.isGestor && !state.isAdmin) return true;
  // Admins só acessam o painel com sessão aal2 (MFA verificado).
  // state.currentAal é setado por auth.js após o enforcement MFA.
  if (state.isAdmin) return state.currentAal === 'aal2';
  return false;
}
